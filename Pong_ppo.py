...
def parse_args():
  ...
  parser.add_argument(
    "--shape", type=int, default=1, help="seed of the experiment"
  )
  parser.add_argument(
    "--total-timesteps",
    type=int,
...
if __name__ == "__main__":
  ...
  for update in range(1, num_updates + 1):
    ...
    for step in range(0, args.num_steps):
      ...
      if args.shape == 0:
        reward -= (a<=1) * 0.003 			#  crazy mode
      if args.shape == 1:
        reward -= (a<=1) * 0.00 			#  natural mode
      if args.shape == 2:
        reward += (a<=1) * 0.003 			#  lazy mode
  ...
  import pickle
  pickle.dump(agent, open('runs/{}_ppo.p'.format(run_name), 'wb'))#runs/{run_name}
  envs.close()
  writer.close()
